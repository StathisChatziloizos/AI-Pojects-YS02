...
class CSP(search.Problem):
    ...
    def support_pruning(self):
        ...
        if self.curr_domains is None:
            # Map each variable to its domain making a set of domains
            self.curr_domains = {v: list(self.domains[v]) for v in self.variables}

    def suppose(self, var, value):
        """Start accumulating inferences from assuming var=value."""
        self.support_pruning()
        # Create a list of all the values in the domain of var different than "value"
        removals = [(var, a) for a in self.curr_domains[var] if a != value]
        # Set vars domain to be just the value
        self.curr_domains[var] = [value]
        return removals

    def prune(self, var, value, removals):
        """Rule out var=value."""
        # Remove value from var's domain and add it to the removals
    ...
    def choices(self, var):
        ...
        # Return the remaining domain of var
        return (self.curr_domains or self.domains)[var]

    def infer_assignment(self):
        """Return the partial assignment implied by the current inferences."""
        self.support_pruning()
        # Return the values of the variables that have only one value left in their domain
        return {v: self.curr_domains[v][0]
                for v in self.variables if 1 == len(self.curr_domains[v])}

    def restore(self, removals):
        """Undo a supposition and all inferences from it."""
        # For each tuple in removals add the value b back to the domain of B
        for B, b in removals:
...
def dom_j_up(csp, queue):
    # Sort the queue according to the amount of available values the second variable of the arc has
...
def first_unassigned_variable(assignment, csp, weights):
...
# Return the remaining values of the variable var
def unordered_domain_values(var, assignment, csp):
    """The default value order."""
    return csp.choices(var)

# Return the value of var's domain that deletes the least values from other variables
